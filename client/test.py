        ...
        for chunk_info in data["chunks"]:
            ...
            for chunk in chunk_info["chunks"]:
                ...
                async with session.get(
                    f"https://autopatchcn.juequling.com/pclauncher/chunks/cxi9diu5aadc/20260414/2.8.0/EyZoSq5h5dpT/{chunk['id']}"
                ) as response:
                    if response.status == 200:
                        ...
                        with zstd.ZstdFile(f"./logs/chunks/{chunk['id']}", "rb") as f:
                            ...
                            decompressed_path = Path(
                                f"./logs/chunks/{chunk['id']}.decompressed"
                            )
                        ...
                        with path.open("ab") as f:
                            print(
                                f"正在写入 {chunk_info['file']} 的 chunk {chunk['id']} {chunk.get('offset', 0)}"
                            )
                            f.seek(int(chunk.get("offset", 0)))
                            with open(
                                f"./logs/chunks/{chunk['id']}.decompressed", "rb"
                            ) as chunk_file:
                                f.write(chunk_file.read())

                    else:
                        print(
                            f"chunk {chunk['id']} 下载失败，状态码: {response.status}"
                        )
